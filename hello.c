...
int main(int argc, char **argv) {
    ...
    double sum = 0.0;
    MPI_Request req;
    MPI_Iallreduce(MPI_IN_PLACE, x, n, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &req);
    ...
    printf("sum = %.1f in rank %3d, wait time (overlap): %7.3f ms, actual reduce time (no overlap): %6.3f ms, "
            "total time: %.3f ms\n",
